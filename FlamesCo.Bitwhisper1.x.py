...
It includes:
  ...
  • Copy message
  • Save transcript
  • Simple "bit" flavored toy model (no heavy weights)

Python 3.9+ recommended. Requires: tkinter (usually bundled), numpy (optional but used here).
...
try:
    ...
    np = None  # The app can still run without numpy

APP_NAME = "BitWhisker0.910b"


# =====================================================
# Toy "bit" model — deterministic, fast, no huge arrays
# =====================================================

class BitWhiskerModel:
    """
    A tiny, offline "language-ish" generator. Not an LLM!
    It synthesizes text with a hash-driven xorshift PRNG and a simple
    character-level language model seeded from the prompt. Temperature
    affects randomness; max_tokens limits length.
    """
    def __init__(self, temperature: float = 0.9, seed: Optional[int] = None):
        self.temperature = max(0.05, min(float(temperature), 1.5))
        # Visible printable range similar to many terminals: ' ' .. '~'
        self.vocab = [chr(i) for i in range(32, 127)]
        self._seed = seed if seed is not None else int(time.time() * 1e6) & 0xFFFFFFFF
        self._state = self._seed

    # --- PRNG (xorshift32) ---
    def _rng_u32(self) -> int:
    ...
    def _rng(self) -> float:
        return (self._rng_u32() & 0xFFFFFF) / float(0x1000000)

    def _hash_prompt(self, text: str) -> int:
        # Fast hash mixing the characters like a bit-scrambler
        h = 0x811C9DC5
        for ch in text:
            h ^= ord(ch)
            h = (h * 0x01000193) & 0xFFFFFFFF
        # blend with seed
        h ^= (self._seed | 0xA5A5A5A5)
        return h & 0xFFFFFFFF

    def generate(self, prompt: str, max_tokens: int = 120, stop_event: Optional[Callable[[], bool]] = None):
        """
        Stream-like generator yielding characters one by one.
        """
        ctx_hash = self._hash_prompt(prompt)
        self._state ^= ctx_hash or 0xDEADBEEF

        # Build a crude frequency bias from prompt characters
        freq = {ch: 1 for ch in self.vocab}
        for ch in prompt[-500:]:  # last 500 chars for locality
            if ch in freq:
                freq[ch] += 2
        ...
        def sample_char(prev: str) -> str:
            # Mix RNG with previous char to get a position-dependent distribution
            r = self._rng()
            # Weighted index selection with temperature
            # We map frequencies -> logits-ish -> softmax via Boltzmann trick
            # To keep it fast and offline, approximate by power transform.
            weights = []
            total = 0.0
            for ch in vocab:
                base = freq.get(ch, 1)
                # perturb by RNG and prev char ordinal
                jitter = 0.75 + 0.5 * self._rng()
                w = (base * jitter) ** (1.0 / max(0.15, temp))
                weights.append(w)
                total += w
            # Normalize and draw
            pick = r * total
            acc = 0.0
            for ch, w in zip(vocab, weights):
                acc += w
                if acc >= pick:
                    return ch
            return vocab[-1]
...
class BitWhiskerApp(tk.Tk):
    ...
    def _apply_theme(self):
        ...
        # Header/Footer backgrounds via direct color
    ...
    def _on_send(self):
        ...
        self._stream_assistant(prompt=text)

    def _stop_stream(self):
        self._stop_flag = True

    def _stream_assistant(self, prompt: str):
        self._stop_flag = False
        ...
        gen = self.controller.model.generate(prompt, max_tokens=self.controller.max_tokens,
                                             stop_event=lambda: self._stop_flag)

        def step():
            nonlocal gen
            try:
                # Emit a few characters per frame to feel "streamy"
                chunk = "".join(next(gen) for _ in range(3))
