...
It includes:
  ...
  • Copy message
  • Save transcript
  • Integration with DeepSeek's R1-Distill-Llama-8B model (distilled variant for local run; requires dependencies and hardware like Apple M4 Pro with MPS)

Python 3.9+ recommended. Requires: tkinter (usually bundled), torch, transformers, accelerate.

Run:
  python bitwhisker_m4pro_port.py
"""
...
# =====================================================
# DeepSeek-R1-Distill-Llama-8B model integration (distilled for local/M4 Pro)
# =====================================================

class BitWhiskerModel:
    """
    Integration with DeepSeek's R1-Distill-Llama-8B model using Hugging Face Transformers.
    Supports streaming and temperature control. Optimized for Apple Silicon (MPS) or CPU.
    Note: Uses Llama-based chat templates.
    """
    def __init__(self, temperature: float = 0.9, seed: Optional[int] = None):
        ...
        # No need for init_empty_weights on smaller distilled model; auto dispatch handles MPS/CPU
        self.model = load_checkpoint_and_dispatch(self.model, model_id, device_map="auto", dtype=dtype)
...
class BitWhiskerApp(tk.Tk):
    ...
    def _build_style(self):
        ...
        # Removed unused TCheckbutton to avoid potential macOS theme issues
    ...
    def _build_layout(self):
        ...
        # Greeting
        self._add_message("assistant", f"{APP_NAME} is online with DeepSeek-R1-Distill-Llama-8B on M4 Pro. Ask me anything!")
    ...
    def _new_chat(self):
        ...
        self._add_message("assistant", f"Started a new conversation in {APP_NAME} with DeepSeek-R1-Distill.")
    ...
    def _open_settings(self):
        ...
        tk.Label(win, text="Settings", font=("Arial", 12, "bold"), bg=pal["bg"], fg=pal["text"]).pack(pady=(10, 6))
